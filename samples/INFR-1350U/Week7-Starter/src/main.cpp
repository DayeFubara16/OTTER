...
int main() {
	...
	shader->SetUniform("u_TextureMix", textureMix);//added
	...
	imGuiCallbacks.push_back([&]() {
		...
		if (ImGui::CollapsingHeader("Material Level Lighting Settings"))
		{
			...
			//added 
			if (ImGui::SliderFloat("Texture Mix", &textureMix, 0.0f, 1.0f)) {
	...
	Texture2DData::sptr diffuseTwoMap = Texture2DData::LoadFromFile("images/box.bmp");//added 
	Texture2DData::sptr specularTwoMap = Texture2DData::LoadFromFile("images/Stone_001_Specular.png");//added
	...
	diffuseTwo->LoadData(diffuseTwoMap);
	Texture2D::sptr specularTwo = Texture2D::Create();
	specularTwo->LoadData(specularTwoMap);
	...
	Texture2D::sptr NewText = Texture2D::Create(desc2);
	NewText->Clear(); 
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader->SetUniform("s_Diffuse2", 2);//added
