...
uniform sampler2D s_Diffuse2; //added
...
uniform float u_TextureMix;//added
...
void main() {
	...
	vec4 textureColorIni = texture(s_Diffuse, inUV);

	//added 
	vec4 textureColorSecond = texture(s_Diffuse2, inUV);

	vec4 textureColor = mix(textureColorIni, textureColorSecond, u_TextureMix);
